...
# =========================================================================
# 边缘设备报警上报（香橙派5Pro / Orange Pi 5 Pro 数据接入）
# =========================================================================
@api_bp.route('/alarm/report', methods=['POST'])
@api_bp.route('/detect/alarm', methods=['POST'])  # 兼容边缘端 flame_alarm.py 默认推送路径
def alarm_report():
    """接收香橙派5Pro边缘端推送的火焰/烟雾报警，转发至PHP后端写入 T_DetectResult。
    上报内容: 检测图片(base64)、3-5秒取证视频(base64/路径)、经纬度位置等；
    详情页(alarm/detail)据此展示火焰图片、视频与发生位置。"""
    ...
    if edge_key:
        ...
        if provided != edge_key:
            return fail('边缘API密钥无效', 403)

    # 转发至PHP后端（/index.php/api/alarm/report，免登录边缘上报接口）
    base = current_app.config.get('PHP_API_BASE', 'http://127.0.0.1:8080/index.php/api')
    url = base.rstrip('/') + '/alarm/report'
    try:
