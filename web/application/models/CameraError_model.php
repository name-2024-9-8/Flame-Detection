<?php
/**
 * 摄像头故障模型 — T_CameraError 查询/维修
 *
 * @author    王永林
 * @studentId 12303070414
 * @created   2026-06-16
 * @modified  2026-06-16
 * @task      M7: 融合修复 — 摄像头故障管理数据层
 */
defined('BASEPATH') OR exit('No direct script access allowed');

class CameraError_model extends CI_Model {
    ...
    public function get_list($page = 1, $per_page = 20, $filters = array()) {
        ...
        if (!empty($filters['camera_id'])) {
            $this->db->where('ce.CameraId', $filters['camera_id']);
        }
        if (!empty($filters['error_code'])) {
            $this->db->where('ce.ErrorCode', $filters['error_code']);
        }

        $total = $this->db->count_all_results('', false);
        $this->db->order_by('ce.CreateTime', 'DESC');
    ...
    public function get_detail($id) {
        $this->db->select('ce.*, c.Name as CameraName');
        $this->db->from('T_CameraError ce');
        $this->db->join('T_Camera c', 'ce.CameraId = c.Id', 'left');
    ...
    public function create($data) {
        $row = array(
            ...
            'CameraIP'   => isset($data['camera_ip']) ? $data['camera_ip'] : null,
        ...
        $this->db->insert('T_CameraError', $row);
    ...
    public function repair($id, $remark = '') {
        ...
        $this->db->where('Id', $id);
        return $this->db->update('T_CameraError', array('Remark' => $remark_text));
    }
    ...
    private function _count_since($since) {
        return $this->db->where('CreateTime >=', $since)
                        ->count_all_results('T_CameraError');
    }
}
