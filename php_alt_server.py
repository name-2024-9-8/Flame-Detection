...
# =====================================================================
# Edge device alarm report (香橙派5Pro / Orange Pi 5 Pro 等边缘端上报)
# =====================================================================
@app.route('/index.php/api/alarm/report', methods=['POST'])
def alarm_report():
    """边缘AI设备(香橙派5Pro / Orange Pi 5 Pro)上报火焰/烟雾报警事件
    接收字段: device_id, camera_id, area_id, longitude, latitude, location,
              confidence, urgency_degree, event_type, description, status,
              picture_base64(检测图片), video_base64(3-5秒取证视频), video_path
    图片/视频保存至 static/uploads/alarms/，记录写入 T_DetectResult，
    供数据大屏与报警详情页查看(检测图片 + 3-5秒视频 + 发生位置)。"""
    import base64, uuid
    ...
    urgency = data.get('urgency_degree') or data.get('UrgencyDegree') or '重要'
    ...
    sql = ("INSERT INTO T_DetectResult "
           "(EventType, Confidence, Longitude, Latitude, Location, Picture, VideoUrl, "
           "AreaId, CreatTime, CameraId, DeviceId, Status, UrgencyDegree, Description) "
           "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)")
    params = (event_type, confidence, str(longitude), str(latitude), location,
              picture_path, video_path, area_id, now, camera_id, device_id,
              str(status), urgency, description)
    new_id = query(sql, params)
    return success({'id': new_id}, '报警上报成功')
