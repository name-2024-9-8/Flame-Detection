...
class APIBridge:
    ...
    def datadict_list(cls, dict_type=None):
        """数据字典列表（从B的T_Dictionary表）"""
        # B没有直接暴露datadict的API，暂时返回空
        # 可以后续在B端扩展 GET /api/statistics/dict
        return {'code': 200, 'msg': 'success', 'data': []}
    ...
    def _map_user(cls, b_user):
        ...
        return {
            'id': b_user.get('id'),
            'username': b_user.get('account', ''),
            'real_name': b_user.get('name', ''),
            'email': b_user.get('email', ''),
            'phone': b_user.get('phone', ''),
            'user_type': 1 if b_user.get('name') == '管理员' else 2,
            'user_type_name': '超级用户' if b_user.get('name') == '管理员' else '普通用户',
            'status': 1,
            'department_id': b_user.get('branch_id'),
            'department_name': '',
            'role_id': None,
            'role_name': '',
            'login_count': 0,
